...
class TCPRelay:
    ...
    def handle_local_stream(self, sock):
        ...
        if self.stage == self.STAGE_INIT:
            ...
            if self.is_client:
                # if the connection is from the local app,
                # insert identity ahead of data
                data = self.cipher.encrypt(data, check=True)
                self.remote_conn.sendall(data)
                self.stage = self.STAGE_STREAM

            else:
                # if the connection is from the client
                # check the identity
                data = self.cipher.decrypt(data, check=True)
